...
from oss2swift.subresource import Owner, encode_acl, ACLPublicRead
...
class TestOss2swiftBucket(Oss2swiftTestCase):
    ...
    def test_bucket_PUT_with_canned_acl(self):
        ...
        self.assertTrue('X-Container-Sysmeta-Oss2swift-Acl' not in headers)
    ...
    def test_bucket_DELETE_error(self):
        ...
        # bucket not empty is now validated at oss2swift
    ...
    def _test_bucket_for_ossacl(self, method, account):
        req = Request.blank('/bucket',
    ...
    def test_bucket_GET_without_permission(self):
        status, headers, body = self._test_bucket_for_ossacl('GET',
                                                            'test:other')
    ...
    def test_bucket_GET_with_fullcontrol_permission(self):
        status, headers, body = \
            self._test_bucket_for_ossacl('GET', 'test:full_control')
        self.assertEqual(status.split()[0], '200')
    ...
    def test_bucket_DELETE_without_permission(self):
        status, headers, body = self._test_bucket_for_ossacl('DELETE',
                                                            'test:other')
        self.assertEqual(self._get_error_code(body), 'AccessDenied')
    ...
    def test_bucket_DELETE_with_fullcontrol_permission(self):
        status, headers, body = \
            self._test_bucket_for_ossacl('DELETE', 'test:full_control')
        self.assertEqual(self._get_error_code(body), 'AccessDenied')
        # Don't delete anything in backend Swift
        called = [method for method, _, _ in self.swift.calls_with_headers]
        self.assertNotIn('DELETE', called)
