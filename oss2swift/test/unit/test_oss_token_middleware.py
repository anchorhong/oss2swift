...
import fixtures
from oss2swift import oss_token_middleware as oss_token
import requests
...
class OssTokenMiddlewareTestBase(unittest.TestCase):

    TEST_AUTH_URI = 'https://fakehost/identity'
    TEST_URL = '%s/v2.0/osstokens' % (TEST_AUTH_URI, )
    TEST_DOMAIN_ID = '1'
    ...
    def setUp(self):
        super(OssTokenMiddlewareTestBase, self).setUp()
        ...
        self.middleware = oss_token.OssToken(self.app, self.conf)
    ...
    def tearDown(self):
        ...
        self.logger.cleanUp()
        super(OssTokenMiddlewareTestBase, self).tearDown()
...
class OssTokenMiddlewareTestGood(OssTokenMiddlewareTestBase):

    def setUp(self):
        super(OssTokenMiddlewareTestGood, self).setUp()
    ...
    def test_without_auth_storage_token(self):
        req = Request.blank('/v1/AUTH_cfa/c/o')
        req.headers['Authorization'] = 'OSS badboy'
        self.middleware(req.environ, self.start_fake_response)
        self.assertEqual(self.response_status, 200)

    def test_authorized(self):
        req = Request.blank('/v1/AUTH_cfa/c/o')
        req.headers['Authorization'] = 'OSS access:signature'
    ...
    def test_authorized_http(self):
        ...
        self.requests_mock.post(
            '%s://%s:%s/v2.0/osstokens' % (protocol, host, port),
            status_code=201, json=GOOD_RESPONSE)

        self.middleware = (
            oss_token.filter_factory({'auth_protocol': 'http',
                                     'auth_host': host,
    ...
    def test_authorized_trailing_slash(self):
        self.middleware = oss_token.filter_factory({
    ...
    def test_authorization_nova_toconnect(self):
        ...
        req.headers['Authorization'] = 'OSS access:FORCED_TENANT_ID:signature'
        req.headers['X-Storage-Token'] = 'token'
        req.get_response(self.middleware)
    ...
    def test_insecure_option(self):
        ...
        for val in true_values:
            ...
            middleware = oss_token.filter_factory(config)(self.app)
    ...
    def test_ipv6_auth_host_option(self):
        ...
        with self.assertRaises(ConfigFileError) as cm:
            oss_token.filter_factory(config)(self.app)
...
class OssTokenMiddlewareTestBad(OssTokenMiddlewareTestBase):
    def test_unauthorized_token(self):
        ...
        oss_denied_req = self.middleware._deny_request('AccessDenied')
        self.assertEqual(resp.body, oss_denied_req.body)
        self.assertEqual(
            resp.status_int,  # pylint: disable-msg=E1101
            oss_denied_req.status_int)  # pylint: disable-msg=E1101
    ...
    def test_bogus_authorization(self):
        ...
        oss_invalid_req = self.middleware._deny_request('InvalidURI')
        self.assertEqual(resp.body, oss_invalid_req.body)
        self.assertEqual(
            resp.status_int,  # pylint: disable-msg=E1101
            oss_invalid_req.status_int)  # pylint: disable-msg=E1101

    def test_fail_to_connect_to_keystone(self):
        with mock.patch.object(self.middleware, '_json_request') as o:
            oss_invalid_req = self.middleware._deny_request('InvalidURI')
            o.side_effect = oss_token.ServiceError(oss_invalid_req)
