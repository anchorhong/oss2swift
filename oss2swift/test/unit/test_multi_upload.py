...
from oss2swift.etree import fromstring, tostring
from oss2swift.request import MAX_32BIT_INT
from oss2swift.subresource import Owner, Grant, User, ACL, encode_acl, \
...
from oss2swift.utils import sysmeta_header, mktime, OssTimestamp
...
objects_template = \
    (('object/X/1', '2014-05-07T19:47:51.000270', 'HASH', 100),
     ('object/X/2', '2014-05-07T19:47:52.000270', 'HASH', 200))

multiparts_template = \
    (('object/X', '2014-05-07T19:47:50.592270', 'HASH', 1),
     ('object/X/1', '2014-05-07T19:47:51.000270', 'HASH', 11),
     ('object/X/2', '2014-05-07T19:47:52.000270', 'HASH', 21),
...
class TestOss2swiftMultiUpload(Oss2swiftTestCase):

    def setUp(self):
        super(TestOss2swiftMultiUpload, self).setUp()
    ...
    def test_bucket_multipart_uploads_GET_without_segment_bucket(self):
        ...
        status, haeaders, body = self.call_oss2swift(req)
    ...
    @ossacl
    @patch('oss2swift.request.get_container_info', lambda x, y: {'status': 404})
    ...
    def test_bucket_multipart_uploads_GET_with_delimiter(self):
        ...
        multiparts = \
            (('object/X', '2014-05-07T19:47:50.592270', 'HASH', 1),
             ...
             ('object/Y/1', '2014-05-07T19:47:51.000270', 'HASH', 21),
             ('object/Y/2', '2014-05-07T19:47:52.000270', 'HASH', 22),
             ('object/Z', '2014-05-07T19:47:50.592270', 'HASH', 3),
             ('object/Z/1', '2014-05-07T19:47:51.000270', 'HASH', 31),
             ('object/Z/2', '2014-05-07T19:47:52.000270', 'HASH', 32),
             ('subdir/object/X', '2014-05-07T19:47:50.592270', 'HASH', 4),
             ('subdir/object/X/1', '2014-05-07T19:47:51.000270', 'HASH', 41),
             ('subdir/object/X/2', '2014-05-07T19:47:52.000270', 'HASH', 42),
             ('subdir/object/Y', '2014-05-07T19:47:50.592270', 'HASH', 5),
             ('subdir/object/Y/1', '2014-05-07T19:47:51.000270', 'HASH', 51),
             ('subdir/object/Y/2', '2014-05-07T19:47:52.000270', 'HASH', 52),
             ('subdir2/object/Z', '2014-05-07T19:47:50.592270', 'HASH', 6),
             ('subdir2/object/Z/1', '2014-05-07T19:47:51.000270', 'HASH', 61),
             ('subdir2/object/Z/2', '2014-05-07T19:47:52.000270', 'HASH', 62))
        ...
        multiparts = \
            (('object/X', '2014-05-07T19:47:50.592270', 'HASH', 1),
             ...
             ('dir/subdir/object/X', '2014-05-07T19:47:50.592270',
              'HASH', 3),
             ('dir/subdir/object/X/1', '2014-05-07T19:47:51.000270',
              'HASH', 31),
             ('dir/subdir/object/X/2', '2014-05-07T19:47:52.000270',
              'HASH', 32),
             ('subdir/object/X', '2014-05-07T19:47:50.592270', 'HASH', 4),
             ('subdir/object/X/1', '2014-05-07T19:47:51.000270', 'HASH', 41),
             ('subdir/object/X/2', '2014-05-07T19:47:52.000270', 'HASH', 42),
    ...
    def test_bucket_multipart_uploads_GET_with_prefix_and_delimiter(self):
        ...
        multiparts = \
            (('dir/subdir/object/X', '2014-05-07T19:47:50.592270',
             ...
             ('dir/object/X/1', '2014-05-07T19:47:51.000270', 'HASH', 51),
             ('dir/object/X/2', '2014-05-07T19:47:52.000270', 'HASH', 52))
    ...
    @patch('oss2swift.controllers.multi_upload.unique_id', lambda: 'X')
    ...
    @patch('oss2swift.controllers.multi_upload.unique_id', lambda: 'X')
    def test_object_multipart_upload_initiate_ossacl(self):
        req = Request.blank('/bucket/object?uploads',
                            environ={'REQUEST_METHOD': 'POST'},
                            headers={'Authorization':
                                     'OSS test:tester:hmac',
    ...
    @ossacl(ossacl_only=True)
    def test_object_multipart_upload_complete_ossacl(self):
    ...
    @ossacl
    @patch('oss2swift.request.get_container_info', lambda x, y: {'status': 204})
    ...
    def _test_for_ossacl(self, method, query, account, hasObj=True, body=None):
    ...
    def test_upload_part_acl_without_permission(self):
        status, headers, body = \
            self._test_for_ossacl('PUT', '?partNumber=1&uploadId=X',
                                 'test:other')
    ...
    def test_list_multipart_uploads_acl_without_permission(self):
        status, headers, body = \
            self._test_for_ossacl('GET', '?uploads', 'test:other',
                                 hasObj=False)
        self.assertEqual(status.split()[0], '403')
    ...
    def test_list_multipart_uploads_acl_with_read_permission(self):
        status, headers, body = \
            self._test_for_ossacl('GET', '?uploads', 'test:read',
                                 hasObj=False)
        self.assertEqual(status.split()[0], '200')
    ...
    def test_list_multipart_uploads_acl_with_fullcontrol_permission(self):
        status, headers, body = \
            self._test_for_ossacl('GET', '?uploads', 'test:full_control',
                                 hasObj=False)
        self.assertEqual(status.split()[0], '200')
    ...
    def test_initiate_multipart_upload_acl_without_permission(self):
        status, headers, body = \
            self._test_for_ossacl('POST', '?uploads', 'test:other')
        self.assertEqual(status.split()[0], '403')
    ...
    def test_initiate_multipart_upload_acl_with_write_permission(self):
        status, headers, body = \
            self._test_for_ossacl('POST', '?uploads', 'test:write')
        self.assertEqual(status.split()[0], '200')
    ...
    def test_initiate_multipart_upload_acl_with_fullcontrol_permission(self):
        status, headers, body = \
            self._test_for_ossacl('POST', '?uploads', 'test:full_control')
        self.assertEqual(status.split()[0], '200')

    @ossacl(ossacl_only=True)
    def test_list_parts_acl_without_permission(self):
        status, headers, body = \
            self._test_for_ossacl('GET', '?uploadId=X', 'test:other')
        self.assertEqual(status.split()[0], '403')

    @ossacl(ossacl_only=True)
    def test_list_parts_acl_with_read_permission(self):
        status, headers, body = \
            self._test_for_ossacl('GET', '?uploadId=X', 'test:read')
        self.assertEqual(status.split()[0], '200')

    @ossacl(ossacl_only=True)
    def test_list_parts_acl_with_fullcontrol_permission(self):
        status, headers, body = \
            self._test_for_ossacl('GET', '?uploadId=X', 'test:full_control')
        self.assertEqual(status.split()[0], '200')

    @ossacl(ossacl_only=True)
    def test_abort_multipart_upload_acl_without_permission(self):
        status, headers, body = \
            self._test_for_ossacl('DELETE', '?uploadId=X', 'test:other')
        self.assertEqual(status.split()[0], '403')

    @ossacl(ossacl_only=True)
    def test_abort_multipart_upload_acl_with_write_permission(self):
        status, headers, body = \
            self._test_for_ossacl('DELETE', '?uploadId=X', 'test:write')
        self.assertEqual(status.split()[0], '204')

    @ossacl(ossacl_only=True)
    def test_abort_multipart_upload_acl_with_fullcontrol_permission(self):
        status, headers, body = \
            self._test_for_ossacl('DELETE', '?uploadId=X', 'test:full_control')
        self.assertEqual(status.split()[0], '204')

    @ossacl(ossacl_only=True)
    def test_complete_multipart_upload_acl_without_permission(self):
        status, headers, body = \
            self._test_for_ossacl('POST', '?uploadId=X', 'test:other',
                                 body=xml)
        self.assertEqual(status.split()[0], '403')
    ...
    def test_complete_multipart_upload_acl_with_write_permission(self):
        status, headers, body = \
            self._test_for_ossacl('POST', '?uploadId=X', 'test:write',
                                 body=xml)
        self.assertEqual(status.split()[0], '200')
    ...
    def test_complete_multipart_upload_acl_with_fullcontrol_permission(self):
        status, headers, body = \
            self._test_for_ossacl('POST', '?uploadId=X', 'test:full_control',
                                 body=xml)
        self.assertEqual(status.split()[0], '200')

    def _test_copy_for_ossacl(self, account, src_permission=None,
        ...
        put_headers = {'Authorization': 'OSS %s:hmac' % account,
                       'Date': self.get_date_header(),
    ...
    def test_upload_part_copy(self):
        ...
        status, headers, body = self._test_copy_for_ossacl(
    ...
    def test_upload_part_copy_acl_invalid_source(self):
        ...
        status, headers, body = \
            self._test_copy_for_ossacl('test:write', 'WRITE', '/bucket')
        self.assertEqual(status.split()[0], '400')
    ...
    def test_upload_part_copy_headers_error(self):
        ...
        status, header, body = \
            self._test_copy_for_ossacl(account,
    ...
    def test_upload_part_copy_headers_with_match(self):
        ...
        status, header, body = \
            self._test_copy_for_ossacl(account, put_header=header)
    ...
    @ossacl(ossacl_only=True)
    def test_upload_part_copy_headers_with_match_and_ossacl(self):
        account = 'test:tester'
        etag = '7dfa07a8e59ddbcd1dc84d4c4f82aea1'
        last_modified_since = 'Fri, 01 Apr 2014 11:00:00 GMT'
        ...
        self.assertEqual(status.split()[0], '200')
        self.assertEqual(len(self.swift.calls_with_headers), 4)
        # Before the check of the copy source in the case of ossacl is valid,
        # oss2swift check the bucket write permissions and the object existence
        # of the destination.
    ...
    @ossacl(ossacl_only=True)
    def test_upload_part_copy_headers_with_not_match_and_ossacl(self):
        account = 'test:tester'
        etag = '7dfa07a8e59ddbcd1dc84d4c4f82aea1'
        last_modified_since = 'Fri, 01 Apr 2014 12:00:00 GMT'
    ...
    def test_upload_part_copy_range_unsatisfiable(self):
        ...
        header = {'X-Oss-Copy-Source-Range': 'bytes=1000-'}
        status, header, body = self._test_copy_for_ossacl(
    ...
    def test_upload_part_copy_range_invalid(self):
        ...
        header = {'X-Oss-Copy-Source-Range': '0-9'}
        status, header, body = \
            self._test_copy_for_ossacl(account, put_header=header)

        self.assertEqual(status.split()[0], '400', body)

        header = {'X-Oss-Copy-Source-Range': 'asdf'}
        status, header, body = \
            self._test_copy_for_ossacl(account, put_header=header)
    ...
    def test_upload_part_copy_range(self):
        ...
        header = {'X-Oss-Copy-Source-Range': 'bytes=0-9'}
        status, header, body = self._test_copy_for_ossacl(
...
class TestOss2swiftMultiUploadNonUTC(TestOss2swiftMultiUpload):
    def setUp(self):
        self.orig_tz = os.environ.get('TZ', '')
        os.environ['TZ'] = 'EST+05EDT,M4.1.0,M10.5.0'
        time.tzset()
        super(TestOss2swiftMultiUploadNonUTC, self).setUp()

    def tearDown(self):
        super(TestOss2swiftMultiUploadNonUTC, self).tearDown()
