# Copyrighy (c) 2015 OpenStack Foundation
#
...
from oss2swift.controllers.multi_delete import MAX_MULTI_DELETE_BODY_SIZE
...
from oss2swift.test.functional.utils import calculate_md5, get_error_code


class TestOss2swiftMultiDelete(Oss2swiftFunctionalTestCase):
    def setUp(self):
    ...
    def test_delete_multi_objects(self):
        ...
        elem = fromstring(body, 'DeleteResult')
        resp_objects = elem.findall('Deleted')
        # oss assumes a NoSuchKey object as deleted.
        self.assertEqual(len(resp_objects), len(req_objects))
        for o in resp_objects:
            self.assertTrue(o.find('Key').text in req_objects)
...
class TestOss2swiftMultiDeleteSigV4(TestOss2swiftMultiDelete):
    @classmethod
    def setUpClass(cls):
