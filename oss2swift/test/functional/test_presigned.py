...
class TestOss2swiftPresignedUrls(Oss2swiftFunctionalTestCase):
    ...
    def test_expiration_limits(self):
        if os.environ.get('Oss_USE_SIGV4'):
    ...
    def _test_expiration_limits_v4(self):
        ...
        self.assertIn('X-Oss-Expires must be non-negative',
                      get_error_msg(resp.content))
        ...
        for exp in (7 * 24 * 60 * 60 + 1,
            ...
            self.assertEqual(get_error_code(resp.content),
                             'AuthorizationQueryParametersError')
            self.assertIn('X-Oss-Expires must be less than 604800 seconds',
                          get_error_msg(resp.content))
        ...
        for exp in (2 ** 63, 'foo'):
            ...
            self.assertEqual(get_error_code(resp.content),
                             'AuthorizationQueryParametersError')
            self.assertEqual('X-Oss-Expires should be a number',
                             get_error_msg(resp.content))
...
class TestOss2swiftPresignedUrlsSigV4(TestOss2swiftPresignedUrls):
    @classmethod
    def setUpClass(cls):
