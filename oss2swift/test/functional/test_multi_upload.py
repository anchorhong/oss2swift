...
from oss2swift.test.functional.utils import get_error_code, get_error_msg
...
class TestOss2swiftMultiUpload(Oss2swiftFunctionalTestCase):
    def setUp(self):
    ...
    def _upload_part_copy(self, src_bucket, src_obj, dst_bucket, dst_key,
        ...
        query = {'partNumber':str(part_num),'uploadId':upload_id}
        req_headers = {'X-Oss-Copy-Source': src_path}
        if src_range:
            req_headers['X-Oss-Copy-Source-Range'] = src_range
        status, headers, body = \
    ...
    def test_object_multi_upload(self):
        ...
        self.assertEqual('http://bucket.oss-ostorage.com/obj1',
...
class TestOss2swiftMultiUploadSigV4(TestOss2swiftMultiUpload):
    @classmethod
    def setUpClass(cls):
    ...
    def test_object_multi_upload_part_copy_range(self):
        if StrictVersion(oss2.__version__) < StrictVersion('3.0'):
            self.skipTest('This stuff got the issue of oss2<=2.x')
